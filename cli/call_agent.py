...
def handle_repl_cmd(cmd: str):
    ...
    if sub == "list":
        ...
        for name, pdef in profiles.items():
            meta = ", ".join([f"{k}={v}" for k, v in (pdef or {}).items()]) or "no filter"
            console.print(f"- {name}  [dim]({meta})[/dim]")
        console.print("- all  [dim](no filter)[/dim]")
        return "ok"
    elif sub == "show":
        cur = read_current_profile() or "all"
        console.print(f"[bold]Current:[/bold] {cur}")
        return "ok"
    elif sub == "set":
        if len(parts) < 3:
...
def main():
    ...
    # REPL
    prof_label = active_profile or "all"
    ...
    while True:
        try:
            ...
            if q.startswith(":"):
                handle_repl_cmd(q)
                continue
            answer, docs, timings, pname, pdef = retrieve_and_answer(q, active_profile, k=args.topk)
            print_answer(answer, docs, timings, pname, pdef)
